...
app.post('/api/verify', async (req, res) => {
    const { agentName, apiEndpoint, capabilities, testLevel, walletAddress } = req.body;
    // Validation
...
async function runVerification(id, endpoint, capabilities, level, walletAddress) {
    const result = verifications.get(id);
    result.status = 'testing';
    verifications.set(id, result);
    try {
        const verification = await (0, verifier_1.verifyAgent)(endpoint, capabilities, level);
        ...
        if (result.status === 'verified' && walletAddress) {
            try {
                const attestation = await (0, attestation_1.createAttestation)(result.agentName, walletAddress, result.score);
